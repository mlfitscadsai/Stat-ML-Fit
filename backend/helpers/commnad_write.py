...
class CommandWriter():
    ...
    def get_command(self, parameters):
        filename = parameters.get("filename", "main.csv")
        code = f"""
        import pandas as pd
        ...
        data = pd.read_csv({json.dumps(filename)})
        ...
        ), "pdp_avgs": partial_dependence_plot_avgs,"fprs":fprs,"tprs":tprs,"auc":aucs[0] if aucs else None,"probas":probas.tolist(), "pdp_grid": partial_dependence_plot_grids,"pfi":features_importance,"manifest": {{"method_name": {json.dumps(parameters.get("method_name"))}, "target": {json.dumps(parameters.get("target"))}, "seed": {json.dumps(parameters.get("seed"))}, "file_name": {json.dumps(filename)}, "status": "completed"}}}})
